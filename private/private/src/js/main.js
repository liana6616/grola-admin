...
jQuery.fn.spbrogatka = function () {
  ...
  function preloadImages(startIndex) {
    ...
    var indices = [(startIndex - 1 + count) % count, startIndex, (startIndex + 1) % count];
  ...
  function updateImage(img, element, container, navDiv, contentDiv, animate) {
    ...
    var title = element.attr('title') ? '<div class="gal_text">' + element.attr('title') + '</div>' : '';
    var cssOptions = {
      width: imgwidth + 'px',
      height: imgheight + 'px',
      'max-width': '100%',
    ...
    if (animate) {
      ...
      setTimeout(function () {
        ...
        // Вставка изображения
        contentDiv.html('<img src="' + element.attr('href') + '" alt="" ' + 'class="gallery-image" style="width:100%; height:100%;" />' + title);
      }, 150);
    } else {
      navDiv.css(cssOptions);
      contentDiv.css(cssOptions);
  ...
  spbrogatkagal.click(function (e) {
    ...
    // Добавление в DOM
    $body.append(background).append(container).append(prevNav).append(nextNav).append(closeBtn).append(swipeLeftArea).append(swipeRightArea);
    ...
    var keyboardNav = setupKeyboardNavigation(currentIndex, container, background, closeBtn, prevNav, navDiv, contentDiv);
...
$(document).ready(function () {
  var selectors = ['[rel=gallery]', '.gallery'];
...
(function () {
  ...
  (i = function () {
    ...
    return t.prototype.set_default_values = function () {
      ...
      var e, s, i, n, o, r, h;
      for (e = "", h = 0, n = 0, o = (r = this.results_data).length; n < o && (s = r[n], i = "", "" !== (i = s.group ? this.result_add_group(s) : this.result_add_option(s)) && (h++, e += i), (null != t ? t.first : void 0) && (s.selected && this.is_multiple ? this.choice_build(s) : s.selected && !this.is_multiple && this.single_set_selected_text(this.choice_label(s))), !(h >= this.max_shown_results)); n++);
      return e;
    ...
    return o(n, e), n.prototype.setup = function () {
      ...
      }(this), this.focus_field.on("keyup.chosen", function (e) {
        return function (s) {
          t(), e.keyup_checker(s);
        };
      }(this)), this.focus_field.on("keydown.chosen", function (e) {
        return function (s) {
      ...
      return this.results_showing && (this.result_clear_highlight(), setTimeout(function (t) {
        return function () {
          return t.focus_field.focus();
        };
      }(this), 0), this.container.removeClass("chosen-with-drop"), this.form_field_jq.trigger("chosen:hiding_dropdown", {
        chosen: this
...
$('body').on('submit', '#login_form', function () {
  var form = $(this);
  var link = $('.visualteam').attr('data-i');
  $.ajax({
    url: '/' + link + '/ajax',
    type: 'POST',
    data: form.serialize(),
    success: function success(response) {
      ...
      if (response.success === true) {
        ...
        var i = modalAlert(response.error.message);
        setTimeout(function () {
          closeModal(i);
        }, 1500);
      }
...
$(document).ready(function () {
  ...
  $('input[type=file]').change(function () {
    var item = $(this);
    var block = item.closest('.input_block');
    var nm = $('label', block).attr('data-v');
    var count = this.files.length;
    var file = declOfNum(count, ['файл', 'файла', 'файлов']);
    var fileCount = 'Выбрано';
    if (count === 1) {
      fileCount = 'Выбран';
    }
    if (count > 0) {
  ...
  if ($('#YMapsID').length) {
    var init = function init() {
      var lat = parseFloat($('#YMapsID').attr('data-lat'));
      var lng = parseFloat($('#YMapsID').attr('data-lng'));
      var myPlacemark;
      if (lat == '0' || lng == '0') {
        lat = parseFloat('59.938784');
        lng = parseFloat('30.314997');
      }
    ...
    var createPlacemark = function createPlacemark(coords) {
...
$('body').on('click', '.catalogPriceAdd', function () {
  var btn = $(this);
  var i = parseInt(btn.attr('data-index'));
...
$('body').on('click', '.pagePaymentServiceAdd', function () {
  var btn = $(this);
  var i = parseInt(btn.attr('data-index'));
  btn.attr('disabled', true).addClass('loading');
  var link = $('.visualteam').attr('data-i');
  $.ajax({
    ...
    success: function success(response) {
      if (response.success && response.html) {
        ...
        alert('Ошибка при добавлении службы доставки');
      }
    },
    error: function error(xhr, status, _error2) {
      console.log(xhr, status);
      console.error('AJAX ошибка:', _error2);
      alert('Произошла ошибка при выполнении запроса');
    },
    complete: function complete() {
      btn.attr('disabled', false).removeClass('loading');
...
// Удаление службы доставки на странице "Оплата и доставка"
$('body').on('click', '.pagePaymentServiceRemove', function () {
  var block = $(this).closest('.pagePaymentServicesItem');
...
// Вкл./выкл. вывод на сайт службы доставки на странице "Оплата и доставка"
$('body').on('click', '.pagePaymentServiceShow', function () {
  var block = $(this).closest('.pagePaymentServicesItem');
  if ($(this).hasClass('active')) {
    $(this).removeClass('active');
    $('.show', block).val(0);
...
$('body').on('click', '.files_delete', function () {
  var item = $(this).closest('.file_card');
  var i = $(this).data('id');
  modalConfirm('Подтвердить удаление?', function () {
    item.hide();
...
function initMigratorScripts() {
  ...
  $(document).on('submit', 'form[data-migration-selector]', function (e) {
    e.preventDefault();
    var form = $(this);
    var action = form.data('migration-selector');
...
function showMigrationSelector(form, action) {
  return new Promise(function (resolve, reject) {
    var pendingMigrations = window.pendingMigrations || [];
    var appliedMigrations = window.appliedMigrations || [];
    var allMigrations = [].concat(_toConsumableArray(pendingMigrations), _toConsumableArray(appliedMigrations)).sort();
    ...
    var modalId = 'migrationSelectorModal';
    ...
    $('#' + modalId).on('click', '.btn-confirm', function () {
      var selectedMigration = $('#' + modalId + ' select').val();
...
function createMigrationSelectorModal(modalId, allMigrations, pendingMigrations, appliedMigrations) {
  ...
  allMigrations.forEach(function (migration) {
    var isPending = pendingMigrations.includes(migration);
    var isApplied = appliedMigrations.includes(migration);
    var status = '';
    if (isPending) status = ' (ожидает)';
    if (isApplied) status = ' (применена)';
    html += "<option value=\"".concat(migration, "\">").concat(migration).concat(status, "</option>");
  });
...
function showAlert(message) {
  ...
  var alertId = 'migratorAlert_' + Date.now();
  var alertClass = type === 'warning' ? 'alert-warning' : type === 'error' ? 'alert-danger' : type === 'success' ? 'alert-success' : 'alert-info';
  var html = "\n        <div id=\"".concat(alertId, "\" class=\"alert ").concat(alertClass, " alert-dismissible fade show\" role=\"alert\" \n             style=\"position: fixed; top: 20px; right: 20px; z-index: 9999; max-width: 400px;\">\n            ").concat(message, "\n            <button type=\"button\" class=\"close\" data-dismiss=\"alert\">\n                <span>&times;</span>\n            </button>\n        </div>\n    ");
  $('body').append(html);
...
function closeModal(i) {
  $(".black".concat(i, ", .mod").concat(i)).remove();
...
$body.on('click', '.no', function () {
  var $modbox = $(this).closest('.modbox');
  var i = $modbox.data('i');
  var type = $modbox.data('type');
...
$body.on('click', '.dialog', function () {
  ...
  var modal = createModal(i, 'dialog');
...
$(document).on('keydown', function (e) {
  if (e.key === 'Escape' || e.keyCode === 27) {
    ...
    if (modals.length > 0) {
      ...
      var i = lastModal.data('i');
      var type = lastModal.data('type');
...
function modalAlert(message, callback) {
  var i = $('.black').length + 1;
  var modal = createModal(i, 'alert');
  var html = "\n        <div class=\"alert-modal\">\n            <div class=\"alert-message\">".concat(message, "</div>\n            <div class=\"alert-buttons\">\n                <button class=\"alert-ok btn\" data-i=\"").concat(i, "\">OK</button>\n            </div>\n        </div>\n    ");
  modal.$modbox.html(html);
...
function modalConfirm(message, confirmCallback, cancelCallback) {
  var i = $('.black').length + 1;
  var modal = createModal(i, 'confirm');
  var html = "\n        <div class=\"confirm-modal\">\n            <div class=\"confirm-message\">".concat(message, "</div>\n            <div class=\"confirm-buttons\">\n                <button class=\"confirm-yes btn\" data-i=\"").concat(i, "\">\u0414\u0430</button>\n                <button class=\"confirm-no btn\" data-i=\"").concat(i, "\">\u041D\u0435\u0442</button>\n            </div>\n        </div>\n    ");
  modal.$modbox.html(html);
...
var fixEmptyParagraphs = function fixEmptyParagraphs(editor) {
...
function initTiny() {
  ...
  tinymce.init({
    ...
    external_plugins: {
      "filemanager": "/vendor/tinymce/tinymce/filemanager/plugin.min.js"
    },
    ...
    menubar: false,
    textcolor_map: ["000000", "Черный", "993300", "Утомленный оранжевый", "333300", "Темно-оливковый", "003300", "Темно-зеленый", "003366", "Темно-лазурный", "000080", "Темно-синий", "333399", "Индиго", "333333", "Очень темно-серый", "800000", "Бордовый", "FF6600", "Оранжевый", "808000", "Оливковый", "008000", "Зеленый", "008080", "Изумрудный", "0000FF", "Синий", "666699", "Серовато-синий", "808080", "Серый", "FF0000", "Красный", "FF9900", "Янтарный", "339966", "Морской зеленый", "33CCCC", "Бирюзовый", "3366FF", "Королевский синий", "800080", "Фиолетовый", "999999", "Средне-серый", "FF00FF", "Пурпурный", "FFCC00", "Золотой", "FFFF00", "Желтый", "00FF00", "Лайм", "00FFFF", "Аква", "00CCFF", "Небесно-голубой", "993366", "Красно-фиолетовый", "FFFFFF", "Белый", "FF99CC", "Розовый", "FFCC99", "Персиковый", "FFFF99", "Светло-желтый", "CCFFCC", "Бледно-зеленый", "CCFFFF", "Бледно-голубой", "99CCFF", "Светло-голубой", "CC99FF", "Слива"],
  ...
  function positionTooltip(element, tooltipText) {
    ...
    var positions = {
      top: {
        ...
        "class": 'top',
        fits: function fits() {
          return this.top > margin && this.left > margin && this.left + tooltipWidth < window.innerWidth - margin;
        }
      },
      bottom: {
        ...
        "class": 'bottom',
        fits: function fits() {
          return this.top + tooltipHeight < window.innerHeight - margin && this.left > margin && this.left + tooltipWidth < window.innerWidth - margin;
        }
      },
      left: {
        ...
        "class": 'left',
        fits: function fits() {
          return this.left > margin && this.top > margin && this.top + tooltipHeight < window.innerHeight - margin;
        }
      },
      right: {
        top: elementRect.top + elementRect.height / 2 - tooltipHeight / 2,
        ...
        "class": 'right',
        fits: function fits() {
          return this.left + tooltipWidth < window.innerWidth - margin && this.top > margin && this.top + tooltipHeight < window.innerHeight - margin;
        }
      }
    };

    // Ищем первую подходящую позицию
    var bestPosition = positions.top;
    ...
    if (bestPosition["class"] === 'top' || bestPosition["class"] === 'bottom') {
      ...
      bestPosition.left = Math.min(bestPosition.left, window.innerWidth - tooltipWidth - margin);

      // Смещаем стрелочку, если подсказка сдвинута
      var arrowOffset = elementRect.left + elementRect.width / 2 - bestPosition.left;
      var maxArrowOffset = tooltipWidth / 2 - arrowSize - 5;
      var clampedArrowOffset = Math.max(-maxArrowOffset, Math.min(arrowOffset, maxArrowOffset));
    ...
    tooltip.removeClass('top bottom left right').addClass(bestPosition["class"]).css({
