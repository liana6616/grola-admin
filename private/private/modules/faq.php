<?php
// Загружаем конфигурацию для FAQ
$config = require_once ROOT . '/config/modules/faq.php';
    ...
    <div class="edit_block">
        <form action='<?= $_SERVER['REDIRECT_URL'] ?>' method='post' class='edit_form' id='edit_form'
            ...
            <?php if ($config['fields']['section_id']['enabled'] ?? false): ?>
                <?= Form::select($config['fields']['section_id']['title'] ?? 'Категория', 'section_id', FaqSections::where('ORDER BY name ASC'), $obj->section_id, true, 'Не выбрано', 'name', 0, '', 0, '') ?>
            <?php endif; ?>

            <?php if ($config['fields']['quest']['enabled'] ?? false): ?>
                <?= Form::input($config['fields']['quest']['title'] ?? 'Вопрос', 'quest', $obj->quest, 1, '', '', '') ?>
            <?php endif; ?>
            
            <?php if ($config['fields']['answer']['enabled'] ?? false): ?>
                <?= Form::textarea($config['fields']['answer']['title'] ?? 'Ответ', 'answer', $obj->answer, 140, '') ?>
            <?php endif; ?>
...
elseif (isset($_POST['add']) || isset($_POST['edit'])) :
    ...
    $obj->quest = ($config['fields']['quest']['enabled'] ?? false) ? trim($_POST['quest'] ?? '') : '';
    $obj->answer = ($config['fields']['answer']['enabled'] ?? false) ? trim($_POST['answer'] ?? '') : '';
...
elseif (isset($_GET['delete'])) :
    ...
    if (!empty($search) && $config['filters']['search']) {
        ...
        if ($config['fields']['quest']['enabled'] ?? false) {
            $searchFields[] = "`quest` like '%{$search}%'";
        }
        if ($config['fields']['answer']['enabled'] ?? false) {
            $searchFields[] = "`answer` like '%{$search}%'";
        }
    ...
    $perPage = $_SESSION['faq']['per_page'] ?? $config['pagination']['default_per_page'];
    ...
    if (!empty($objs)): ?>
        <div class="table_container">
            ...
            <?php foreach ($objs as $obj): 
                // Загружаем категорию, если она нужна
                $section = null;
                if (($config['list']['section']['enabled'] ?? false) && ($config['fields']['section_id']['enabled'] ?? false) && !empty($obj->section_id)) {
                    $section = FaqSections::findById($obj->section_id);
                }
            ?>
