<?php
// Загружаем конфигурацию для "Почему выбирают нас"
$config = require_once ROOT . '/config/modules/why_choose_us.php';
...
elseif (isset($_POST['add']) || isset($_POST['edit'])) :
    ...
    if ($config['fields']['image']['enabled'] ?? false) {
        ...
        $path = '/public/src/images/why_choose_us/';
...
elseif (isset($_GET['delete'])) :
    ...
    $perPage = $_SESSION['why_choose_us']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id ASC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: WhyChooseUs::class,
        where: $where,
        order: $order_by,
