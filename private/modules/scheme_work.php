<?php
// Загружаем конфигурацию для "Схема работы"
$config = require_once ROOT . '/config/modules/scheme_work.php';
...
elseif (isset($_GET['delete'])) :
    ...
    $perPage = $_SESSION['scheme_work']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id ASC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: SchemeWork::class,
        where: $where,
        order: $order_by,
