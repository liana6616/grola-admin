<?php
// Загружаем конфигурацию для "Типы сортировки каталога"
$config = require_once ROOT . '/config/modules/catalog_sorts.php';

use app\Models\CatalogSorts;
...
if (isset($_GET['add']) || isset($_GET['edit']) || isset($_GET['copy'])) :
    ...
    <div class="edit_block">
        <form action='<?= $_SERVER['REDIRECT_URL'] ?>' method='post' class='edit_form'>
            ...
            <?php if ($config['fields']['order_by']['enabled'] ?? false): ?>
                <?= Form::input($config['fields']['order_by']['title'] ?? 'SQL выражение для сортировки', 'order_by', $obj->order_by, 1, 'text', '', '') ?>
                <small>Пример: price ASC, id DESC</small>
            <?php endif; ?>
...
elseif (isset($_POST['add']) || isset($_POST['edit'])) :
    ...
    if ($config['fields']['order_by']['enabled'] ?? false) {
        $obj->order_by = trim($_POST['order_by'] ?? '');
    }
...
elseif (isset($_GET['delete'])) :
    ...
    if (!empty($search) && $config['filters']['search']) {
        ...
        if ($config['fields']['order_by']['enabled'] ?? false) {
            $searchFields[] = "`order_by` like '%{$search}%'";
        }
    ...
    $perPage = $_SESSION['catalog_sorts']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id ASC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: CatalogSorts::class,
        where: $where,
        order: $order_by,
    ...
    if (!empty($objs)): ?>
        <div class="table_container">
            <div class="table_header">
                ...
                <?php if (($config['list']['order_by']['enabled'] ?? false) && ($config['fields']['order_by']['enabled'] ?? false)): ?>
                    <div class="pole description"><?= $config['list']['order_by']['title'] ?? 'Сортировка' ?></div>
                <?php endif; ?>
                ...
                <div class="table_row" data-id="<?= $obj->id ?>" data-class="<?= get_class($obj) ?>">
                    ...
                    <?php if (($config['list']['name']['enabled'] ?? false) && ($config['fields']['name']['enabled'] ?? false)): ?>
                        <div class="pole info">
                            ...
                            <div class="name"><?= htmlspecialchars($obj->name) ?></div>
                        </div>
                    <?php endif; ?>
                    
                    <?php if (($config['list']['order_by']['enabled'] ?? false) && ($config['fields']['order_by']['enabled'] ?? false)): ?>
                        <div class="pole description">
                            <div class="title"><?= $config['list']['order_by']['title'] ?? 'Сортировка' ?></div>
                            <div class="text"><?= htmlspecialchars($obj->order_by) ?></div>
                        </div>
                    <?php endif; ?>
