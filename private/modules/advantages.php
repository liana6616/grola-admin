<?php
// Загружаем конфигурацию для преимуществ
$config = require_once ROOT . '/config/modules/advantages.php';
...
if (isset($_GET['add']) || isset($_GET['edit']) || isset($_GET['copy'])) :
    ...
    <div class="editHead">
        ...
        <div class="button_block">
            ...
            <?php if (!empty($pageUrl) && $config['actions']['open']): ?>
    ...
    <div class="edit_block">
        <form action='<?= $_SERVER['REDIRECT_URL'] ?>' method='post' class='edit_form'
            ...
            <?php if ($config['fields']['name']['enabled'] ?? false): ?>
                <?= Form::input($config['fields']['name']['title'] ?? 'Название преимущества', 'name', $obj->name, 0, '', '', '') ?>
            <?php endif; ?>
...
elseif (isset($_POST['add']) || isset($_POST['edit'])) :
    ...
    if ($config['fields']['image']['enabled'] ?? false) {
        ...
        $path = '/public/src/images/advantages/';
...
elseif (isset($_GET['delete'])) :
    ...
    $perPage = $_SESSION['advantages']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id ASC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: Advantages::class,
        where: $where,
        order: $order_by,
