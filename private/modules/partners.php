<?php
// Загружаем конфигурацию для партнёров
$config = require_once ROOT . '/config/modules/partners.php';
...
if (isset($_GET['add']) || isset($_GET['edit']) || isset($_GET['copy'])) :
    ...
    if(!empty($_GET['copy']) && $config['actions']['copy']) {
        $id = $_GET['copy'];
        $title = 'Копирование партнёра';
    }
    
    if(!empty($id)) {
        $obj = Partners::findById($id);
        if (!$obj) {
    ...
    if(!empty($_GET['copy']) && $config['actions']['copy']) {
        // Очищаем логотип при копировании
        $obj->image = '';
        $id = false;
    }
    ...
    <div class="edit_block">
        <form action='<?= $_SERVER['REDIRECT_URL'] ?>' method='post' class='edit_form'
            ...
            <?php if ($config['fields']['name']['enabled'] ?? false): ?>
                <?= Form::input($config['fields']['name']['title'] ?? 'Название партнёра', 'name', $obj->name, 0, '', '', '') ?>
            <?php endif; ?>
            
            <?php if ($config['fields']['link']['enabled'] ?? false): ?>
                <?= Form::input(
                    $config['fields']['link']['title'] ?? 'Ссылка на сайт', 
                    'link', 
                    $obj->link, 
                    0, 
                    '', 
...
elseif (isset($_GET['delete'])) :
    ...
    // Удаление логотипа, если он есть
    ...
    $perPage = $_SESSION['partners']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id DESC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: Partners::class,
        where: $where,
        order: $order_by,
        ...
        <div class="table_container">
            <div class="table_header">
                ...
                <?php if (($config['list']['image']['enabled'] ?? false) && ($config['fields']['image']['enabled'] ?? false)): ?>
                    <div class="pole image_preview"><?= $config['list']['image']['title'] ?? 'Логотип' ?></div>
                <?php endif; ?>
            ...
            <?php foreach ($objs as $obj): 
                $pageUrl = $obj->link;
                $linkTitle = 'Перейти на сайт партнёра';
            ?>
                ...
                <div class="table_row" data-id="<?= $obj->id ?>" data-class="<?= get_class($obj) ?>">
                    ...
                    <?php if (($config['list']['image']['enabled'] ?? false) && ($config['fields']['image']['enabled'] ?? false)): ?>
                        <div class="pole image_preview">
                            <div class="title"><?= $config['list']['image']['title'] ?? 'Логотип' ?></div>
                            <?php if (!empty($obj->image)): ?>
                                <img src="<?= $obj->image ?>" alt="<?= htmlspecialchars($obj->name, ENT_QUOTES, 'UTF-8') ?>">
                            <?php else: ?>
                                <div class="no-image">Нет логотипа</div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
