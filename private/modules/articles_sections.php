<?php
// Загружаем конфигурацию для разделов статей
$config = require_once ROOT . '/config/modules/articles_sections.php';
...
// Разделы статей не поддерживают черновики
$useDrafts = false;

if (isset($_GET['add']) || isset($_GET['edit']) || isset($_GET['copy'])) :

    $obj = new ArticlesSections();
    ...
    if(!empty($id)) {
        $obj = ArticlesSections::findById($id);
        if (!$obj) {
...
elseif (isset($_GET['delete'])) :
    ...
    $obj = ArticlesSections::findById($_GET['delete']);
    ...
    $perPage = $_SESSION['articlessections']['per_page'] ?? $config['pagination']['default_per_page'];
    $order_by = $config['pagination']['order_by'] ?? 'ORDER BY rate DESC, id DESC';

    // Вызов пагинации
    $result = Pagination::create(
        modelClass: ArticlesSections::class,
        where: $where,
        order: $order_by,
