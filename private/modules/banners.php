<?php
// Загружаем конфигурацию для баннеров
$config = require_once ROOT . '/config/modules/banners.php';
        ...
        <form action='<?= $_SERVER['REDIRECT_URL'] ?>' method='post' class='edit_form'
            ...
            <?php if ($config['fields']['image']['enabled'] ?? false): ?>
                <?= Form::image(
                    ...
                    ($config['fields']['image']['width'] ?? 1380) . '×' . 
                    ($config['fields']['image']['height'] ?? 550) . ')', 
                    'image', 
                    $obj, 
                    '', 
                    0
            ...
            <?php if ($config['fields']['video']['enabled'] ?? false): ?>
                <?= Form::file(
                    $config['fields']['video']['title'] ?? 'Видео файл', 
                    'video', 
                    $obj, 
                    '', 
                    'accept="video/*"'
            ...
            <?php if ($config['fields']['price']['enabled'] ?? false): ?>
                <?= Form::input($config['fields']['price']['title'] ?? 'Стоимость', 'price', $obj->price, 0, '', '', '') ?>
            <?php endif; ?>

            <?php if ($config['fields']['button']['enabled'] ?? false): ?>
                <fieldset class="input_block flex2 top">
                    <legend><?= $config['fields']['button']['block_name'] ?? 'Кнопка' ?></legend>
                    <?php if ($config['fields']['button']['name']['enabled'] ?? false): ?>
                        <?= Form::input($config['fields']['button']['name']['title'] ?? 'Текст на кнопке', 'button_name', $obj->button_name, 0, '', '', '') ?>
                    <?php endif; ?>
                    
                    <?php if ($config['fields']['button']['link']['enabled'] ?? false): ?>
                        <?= Form::input($config['fields']['button']['link']['title'] ?? 'Ссылка с кнопки', 'button_link', $obj->button_link, 0, '', '', 'placeholder="Например: /catalog или https://example.com"') ?>
                    <?php endif; ?>
                </fieldset>
            <?php endif; ?>
...
elseif (isset($_POST['add']) || isset($_POST['edit'])) :
    ...
    if ($config['fields']['price']['enabled'] ?? false) {
        $obj->price = trim($_POST['price'] ?? '');
    }
    ...
    if (($config['fields']['button']['enabled'] ?? false) && ($config['fields']['button']['name']['enabled'] ?? false)) {
        $obj->button_name = trim($_POST['button_name'] ?? '');
    }
    
    if (($config['fields']['button']['enabled'] ?? false) && ($config['fields']['button']['link']['enabled'] ?? false)) {
    ...
    if ($config['fields']['image']['enabled'] ?? false) {
        $width = $config['fields']['image']['width'] ?? 1380;
        $height = $config['fields']['image']['height'] ?? 550;
...
elseif (isset($_GET['delete'])) :
    ...
    if (!empty($search) && $config['filters']['search']) {
        ...
        if ($config['fields']['price']['enabled'] ?? false) {
            $searchFields[] = "`price` like '%{$search}%'";
        }
    ...
    $perPage = $_SESSION['banners']['per_page'] ?? $config['pagination']['default_per_page'];
        ...
        <div class="table_container">
            <div class="table_header">
                ...
                <?php if (($config['list']['image']['enabled'] ?? false) && ($config['fields']['image']['enabled'] ?? false)): ?>
                    <div class="pole image_big"><?= $config['list']['image']['title'] ?? 'Баннер' ?></div>
                <?php endif; ?>
